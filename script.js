...
const getCartProducts = (async()=>{
    for(let i = orderList.length-1; i < orderList.length; i++){
        const cartProducts = await(await fetch(`/api/products/${orderList[i]}`)).json();
        let test = cartProducts.id
        console.log(checkDuplicate.includes((test).toString()))
        checkDuplicate.push((test).toString())
        // if(!orderList.includes((i).toString())){
        //     // cartProducts[orderList.indexOf((i).toString())].quantity = "test"
        // }else{
        //     cartItems.push(cartProducts)
        //     cartItems[i].quantity = localStorage.getItem(orderList[i])
        // }
